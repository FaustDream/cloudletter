...
| 单测文件 | 与源同目录 `*.test.ts` | `serializer.test.ts`、`paste-detect.test.ts` | 🔴 |
...
│  ├─ middleware/         # auth.ts（requireAuth/requireRole）、rate-limit.ts
│  ├─ routes/             # v2.ts（唯一路由聚合点）
...
> 序列化契约（Markdown ↔ Block[]）为**内容格式契约**，由唯一生产者/消费者 `apps/write/src/lib/serializer.ts` 承载（编辑 + 预览），服务端 `sync.ts` 以 `rawMarkdown` 原样落盘。禁止在 `server/` 重复实现导致双源漂移（已清理历史死代码）。

### 2.3 `apps/{write,console}/`（前端 SPA）
```
apps/<name>/
├─ src/
│  ├─ main.tsx / App.tsx  # 入口 + 路由（BrowserRouter basename=/dev/<name>/）
│  ├─ api.ts              # API 客户端（错误包络解析）
│  ├─ auth.tsx            # 认证上下文（Bearer 持久化）
│  ├─ components/         # PascalCase 组件
│  ├─ editor/ lib/        # 编辑器逻辑 / 工具（含 *.test.ts）
│  └─ styles.css
...
```
e2e/
├─ tests/*.spec.ts        # auth / console / write 分文件
...
- **鉴权**：所有写接口在 `requireAuth` + 最小 `requireRole` 之后；认证端点接入 `rate-limit`。🔴
- **输入校验**：用 **zod** 替代散落 `if(!x)`；`PUT /settings` 仅允许已知分组（未知分组 422）。🟡
...
- **SPA 子路径**：`apps/write` → `/dev/write/`，`apps/console` → `/dev/console/`；`BrowserRouter basename` 对齐，nginx 反代一致。🔴
- **三态**：所有 `fetch`/异步调用具备 loading / error / empty；错误文案对用户友好（中文）。🔴
- **性能预算**：重依赖（CodeMirror/Tiptap）**懒加载分包**；首屏 JS 受控（write 首屏 ~279KB / gzip 108KB 为基线）。🟡
...
- **认证**：scrypt + `timingSafeEqual` + TOTP(±1)；改密后撤销其他会话；新口令长度/复杂度校验。🔴
- **限流**：认证端点 IP + 账户双维度限流（5min×5，锁 15min）。🔴
