...
- 2026-09-07：编辑器工具栏大改版 + 随手记三项修复并已上站。① 随手记「打字撤销/丢字」：编辑器新增非受控模式（value 仅初值、编辑中外部不回灌），随手记接入，根治回灌整篇替换类撤销；同源路径排查无其它同类问题。② 随手记高度自适应：内容底边=保存按钮上方分割线，无多余空白。③ 随手记标签/分类改 Notion 式下拉（多选标签+单选分类，均可新建落库）。④ 工具栏全面重制：全部按钮换尖锐感消除的 Lucide 风格线性 SVG 图标；标题改正文/H1-H6 下拉；文字色/背景色补「默认黑/默认白」正常色；行内代码与代码块并邻；新增 折叠列表 + 无序（圆点/圆圈/方块）/有序（数字/字母/大写/罗马）标记样式切换（listStyle prop + data 属性 CSS + markdown HTML 注释持久化 lib/listStyles，TDD 12 例）；新增 缩进/减进、清除格式；链接改弹层输 URL（弃 prompt）；图片支持 本地上传/粘贴链接；视频支持 本地上传（video 块直接播放）+ 在线链接（B站/YouTube/优酷/腾讯自动内嵌播放，其余链接卡）；后端 uploads 扩展 mp4/webm/ogg 视频直传（魔数嗅探+视频配额）。前端单测 122/122、后端 129/129、双端 typecheck 与构建通过。
