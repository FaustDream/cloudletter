...
- 2026-09-08（四）：总览三项体验调整并已上站。① 类型筛选由一排胶囊改为多选下拉（`TypeFilter`：全部/日志/灵感/计划/习惯/记账/目标/专注，可勾选多项，「或」关系过滤），`filterNodes` 签名改为 `TimelineType[]`（空=全部），四视图（时光/神经/城市/核心）同步适配，新增/更新单测 3 例。② 速记按钮折叠为 46px 圆形纯 ⚡ 图标、贴左下边停靠（不再遮文字），拖拽/双击复位保留，拖拽边界按新尺寸收紧。③ 个人中心菜单增强实底（`--surface-solid` 不透明）+ 双层柔和阴影，侧边栏弹出清晰可读。验证：typecheck 干净、前端单测 131/131、真实浏览器确认多选筛选/纯图标 FAB/菜单实底；部署（worktree 干净出包）入口 index-zo7eYvP6.js 与本地一致、healthz ok。
- 2026-09-08（三）：总览世界导航移除提示文案「同一份数据 · 四种解释方式」（`.world-nav-tip` 元素与样式一并删除）。部署（worktree 干净出包）入口 index-CUd99g1q.js 与本地一致、healthz ok；本次将「解包前先清旧前端产物」写入部署脚本，线上 assets 无旧 chunk 残留。
- 2026-09-08（二）：修复 SMTP 发送失败被包络为笼统 500 的问题并已上站。`sendMail` 对网络/TLS/认证异常（如 QQ 邮箱 535 授权码失效）此前脱敏后抛出 → 前端只见「服务器内部错误」；现改为返回 `sent:false + reason`，六个邮件调用点（发码/解锁/重置/2FA 恢复/反馈/授权邀请）已有的 503 分支统一生效，失败原因明文返回前端。忘记密码弹窗：去掉右上角 ×（Esc/遮罩/页脚关闭保留）、发送失败原因就地显示在弹窗内（此前写在弹窗后被遮住，表现为「无反应」）。单测新增 mailer 2 例（137/137）；部署备份 `2026-09-08T02-17-11`，生产实测 send-reset 返回 503+535 明文原因。注：535 为 QQ 邮箱 SMTP 账号/授权码问题，需在 QQ 邮箱侧检查，代码仅保证失败反馈清晰。
- 2026-09-08：总览四世界修复与日期筛选整合。① 修复「时光/神经/城市/核心」导航样式失效——双根因：`main.css` `.world-nav` 规则括号错位（background/border-radius/box-shadow 等成为规则外孤儿声明被 CSS 静默丢弃）＋ `.main` 去容器化把 `--surface-card`/`--shadow-card` 覆写为透明/无阴影；导航条改用不随覆写的 `--surface-nested` 玻璃背景 + 固定阴影 + blur 恢复（校验全文件无孤儿声明、括号配平）。② 时光长河滚轮不再切换时间档位（移除 `onRiverWheel` 拦截，滚轮恢复自然翻页）；顶栏「当天/近7天/近30天/本月/近一年」档位按钮组删除，快捷档位收进日期控件（RangePicker：当天/近7天/近30天/本月/近一年，`quickStateOf(v)` 由档位构造区间，原 60 天档移除）；日期控件图标由 emoji 换成内联日历 SVG（7 path，fill #03A9F4）。验证：typecheck 干净、前端单测 129/129、真实浏览器确认导航条背景/阴影/圆角/blur 恢复、视图切换正常、滚轮滚动日期控件文字不再变化。部署（worktree 干净出包，避免带上在途 WIP）：入口 index-DumZT5GR.js 与本地一致、healthz ok、/posts /overview 200。
- 2026-09-08：修复邮件内链接指向 API 端口的问题并已上站。忘记密码重置 / 临时授权邀请 / RSS Feed 的站点 origin 此前直接取 API 请求 Host——本地 vite 代理（changeOrigin）把 Host 改写为后端端口，邮件里的 `/login?reset=...` 打到 Express 返回应用级 404「接口不存在」；生产因 nginx `proxy_set_header Host $host` 传真实域名而一直正常。新增 `server/src/lib/webOrigin.ts`（WEB_ORIGIN env > 请求 Origin 头 > Host 兜底）三处统一接入；单测新增 6 例（135/135）。部署备份 `2026-09-08T01-38-08`（db push no-op），公网哈希 index-DABrgZXx.js 不变，RSS feed 链接实测随 Origin 正确生成。
- 2026-09-08：本地默认管理员口令更改为 `123456`。`server/src/seed.ts` 口令策略调整：本地（非生产）未设置 `ADMIN_PASSWORD` 时默认使用 123456（长度要求放宽至 6 位、绕过弱口令黑名单）；已有管理员口令不一致时自动同步为新口令（等价于重置，本地忘记口令执行 `pnpm seed` 即可恢复默认）；生产（`NODE_ENV=production`）维持原约束——必须显式提供 ≥8 位且非弱口令，禁止默认凭据上线。已执行 seed 将本地管理员（935411629@qq.com）口令同步为 123456，并校验散列一致、seed 幂等（重跑不重复重置）。
- 2026-09-07（四）：随手记三项问题根治并已上站。① 打字被撤销根因确诊：灵感笔记编辑抽屉与速记汇总编辑弹窗此前仍是受控编辑器——受控链路里唯一会「无用户操作改写正文」的路径是外部值回灌 replaceBlocks（且写入 ProseMirror 撤销历史，表现即触发撤销），300ms 防抖上抛 × 异步序列化 × 父组件 setState 交织时 `value===lastEmitted` 短路可能错位，回灌一旦发生即重置输入并可往复循环；两处编辑表单改 `uncontrolled`（value 仅初值，与速记弹窗同款），编辑会话内回灌路径彻底不存在。② 分类/标签改 Notion 式下拉：TagMultiSelect 接入灵感抽屉与速记汇总弹窗（标签多选=全站标签池、分类单选，均可搜索+直接新建落库；分类名→id 解析收敛为 lib/taxonomy `resolveCategoryIdByName`，QuickNoteModal 同步复用），单测新增 4 例。③ 编辑区高度自适应：全局 `.bn-container` 300px 最小高在抽屉/表单弹窗内压平（min-height 120），编辑器 minHeight 380/260→120，正文底边贴合保存按钮上方分割线、随内容增长。验证：typecheck 干净、前端单测全绿、vite build 通过；部署备份 `2026-09-07T08-20-16` → db push no-op → 公网哈希 index-DABrgZXx.js 与本地一致，/ /posts /notes 均 200、API 鉴权包络正常。
- 2026-09-07：编辑器工具栏大改版 + 随手记三项修复并已上站。① 随手记「打字撤销/丢字」：编辑器新增非受控模式（value 仅初值、编辑中外部不回灌），随手记接入，根治回灌整篇替换类撤销；同源路径排查无其它同类问题（真实浏览器逐键复测：中文输入、空格、退格、方向键均不触发撤销）。② 随手记高度自适应：内容底边=保存按钮上方分割线（含覆写 BlockNote 容器 300px 最小高与正文区 64px 底内边距，弹窗内收紧到 120px 最小高 + 4px 底内边距），无多余空白。③ 随手记标签/分类改 Notion 式下拉（多选标签+单选分类，均可新建落库）。④ 工具栏全面重制：全部按钮换尖锐感消除的 Lucide 风格线性 SVG 图标；标题改正文/H1-H6 下拉；文字色/背景色补「默认黑/默认白」正常色；行内代码与代码块并邻；新增 折叠列表 + 无序（圆点/圆圈/方块）/有序（数字/字母/大写/罗马）标记样式切换（listStyle prop + data 属性 CSS + markdown HTML 注释持久化 lib/listStyles，TDD 12 例）；新增 缩进/减进、清除格式；链接改弹层输 URL（弃 prompt）；图片支持 本地上传/粘贴链接；视频支持 本地上传（video 块直接播放）+ 在线链接（B站/YouTube/优酷/腾讯自动内嵌播放，其余链接卡）；后端 uploads 扩展 mp4/webm/ogg 视频直传（魔数嗅探+视频配额）。前端单测 122/122、后端 129/129、双端 typecheck 与构建通过。
- 2026-09-07：灵感笔记增加使用状态（待使用/已使用/已过期）。① `NoteItem` 新增 `status` 字段（默认 `pending`，`purge` 非破坏性加列）；后端 notes 白名单/校验层（`WB_SCHEMA.notes` oneOf 校验、`FIELDS.notes`、createNote/updateNote 缺省 pending），非法状态 422。② 灵感页卡片元信息行新增状态徽标（待使用=信息蓝 / 已使用=成功绿 / 已过期=警示琥珀），页头新增「全部/待使用/已使用/已过期」状态筛选；编辑抽屉分类/状态/标签三列并排，可随时改状态。③ 时间轴灵感节点：非待使用状态在预览前加前缀（如「已使用 · …」）。④ 单测新增：创建缺省 pending、更新 used/expired、非法值 422、时间轴状态前缀，全量 127/127（后端）、98/98（前端）、双端 typecheck 与构建通过。
- 2026-09-06（三）：总览四世界八项反馈修复并已上站。① 时光长河移除悬浮「选择日期」胶囊浮窗（可拖拽日期定位 + 自绘月历弹层整组下线，代码/样式/位置记忆清理）。② 修复滑到「当天」无数据时河面空白且滚轮卡死：滚轮容器 `.river-scroll` 常驻、空态留在河面内，任意数据状态下滚轮都能切回更大的时间尺度（当天→近7天→近30天→本月→近一年循环）。③ 右上角头像旁经验/金币胶囊撤下，HUD 只留头像（与「设置→游戏」开关门控合流，默认仅头像）。④ 节点宇宙新增流星粒子：7 颗随机陨石拖尾循环划过（加色混合亮度包络，尊重 prefers-reduced-motion 与帧降质逻辑）。⑤ 修复神经模式左下角监控 CPU/网络恒 0：后端 `/server-status` 网络正则错抓 tx_packets 列改回 tx_bytes（恒 0 根因）、up/down 与 rx/tx 对调、CPU 改跨请求长窗口差值采样；前端 CPU 保留一位小数、网络箭头对调为 ↓下载/↑上传。⑥ 移除神经模式左上角「← 时光长河」返回快捷按钮（世界切换走顶部四世界导航）。⑦ 数字城市全屏化：`.city-stage` fixed 铺满视口替代原 100dvh-200px 圆角容器，星空/网格背景无限延伸，新增火柴人动画小人（计划区旁工人挥锤 + 行人穿城，reduced-motion 降级为静立）。⑧ 数据核心同步全屏化（fixed 铺满 + 点阵星空延展，HUD 标签下移避开顶栏）。服务端单测 125/125、前端 98/98、双端 typecheck 与构建通过。部署：备份 `2026-09-06T04-39-23` → 上站 → healthz ok、公网哈希 index-CdlLa0el.js 与本地一致、/ /posts /overview 均 200、server-status 返回鉴权包络。
