...
interface RevDetail extends RevMeta {
  rawMarkdown: string
}
...
export function RevisionPanel({
  ...
  const restore = async () => {
    ...
    try {
      ...
      onRestored(selected.rawMarkdown)
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e))
    } finally {
      setBusy(false)
  ...
  const diff = selected ? lineDiff(selected.rawMarkdown, currentMarkdown) : []
      ...
      <div className="rev-dialog" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="rev-body">
          <div className="rev-list">
            {items.length === 0 && <div className="empty">暂无历史版本</div>}
            {items.map((r) => (
              <button
                key={r.id}
          ...
          <div className="rev-diff">
            {busy && <div className="empty">载入中…</div>}
