...
- **每次需求改动，除提交到 git 外，必须按 `docs/DEPLOY.md` 部署到生产服务器**——用户以服务器上的实际效果为准，未部署等于改动未完成。
- 部署前置门禁：改动涉及包的 `typecheck`、单测、前端 `build` 全绿；工作区已 commit（所有待部署改动入库）。
- 部署硬性顺序（详见 `docs/DEPLOY.md`）：**先备份（`pnpm backup`）→ 打包上传 → 依赖安装 → 库结构同步 → 前端产物 → systemd/nginx → 重启 → healthz 探活**。
- 涉及数据库结构变更时，必须按 `docs/DEPLOY.md` §4 的「中间态 schema → 一次性迁移脚本 → 终态 schema」三段执行，禁止直接推会删列的终态 schema。
- 服务器地址走本机 `~/.ssh/config` 的 `cloudletter` 别名；**禁止把服务器 IP、密钥、`.env.production` 内容写入仓库**。
- 部署完成后必须做验证（healthz + 公网产物哈希/接口抽查 + 改动相关页面人工核对），并把结果写进回复与 memory 记录。
- 线上与本地表现不一致时，以**服务器实际效果**为准排查（先核对线上产物版本与部署时间）。
